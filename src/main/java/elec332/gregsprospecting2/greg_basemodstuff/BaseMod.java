...
public class BaseMod implements IGuiHandler {
	...
	public BaseMod() {
		base = this;
		modPackage = getClass().getPackage().getName();
		//String resourceRelDir = modPackage.replace(".", "/") + "/resources/";
		String resourceRelDir = "assets/" + modPackage + "/";
	...
	public <ITEM extends Item> ITEM newItem(String name, Class<ITEM> cls, String title) {
		try {
			...
			String qualName = modPackage + ":" + name;
		...
		int updateFrequency, boolean sendVelocityUpdates)
	{
		...
		EntityRegistry.registerModEntity(cls, name, id, base, 256, updateFrequency, sendVelocityUpdates);
	}
	...
	public ResourceLocation resourceLocation(String path) {
		return new ResourceLocation(modPackage, path);
	}
